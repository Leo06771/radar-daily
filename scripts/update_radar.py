...
FEEDS = [
    ...
    # 财联社用 depth/hot 路由（电报 telegraph 路由的 item 无原文链接，v6 不予收录）
    ("财联社·深度(RSSHub)", "https://rsshub.app/cls/depth"),
    ("财联社·深度(镜像)", "https://rsshub.rssforever.com/cls/depth"),
    ("财联社·热门(RSSHub)", "https://rsshub.app/cls/hot"),
    ("财联社·热门(镜像)", "https://rsshub.rssforever.com/cls/hot"),
...
RULES = [
    ...
    ("一级宏观", "顶层政策规划", ["政治局", "国常会", "国务院常务会议", "十五五", "五年规划", "全会", "会议指出", "中央经济工作会议", "发改委", "工信部", "财政部", "商务部", "金融监管总局"]),
    ...
    ("三级资金面", "盘面快讯", ["A股", "沪指", "上证", "深证", "创业板", "科创50", "北证", "恒指", "恒生", "收涨", "收跌", "成交额", "午评", "收评", "龙虎榜", "涨停", "跌停", "大宗交易", "停牌", "复牌"]),
    ...
    ("四级个股龙头", "业绩预告与暴雷", ["业绩预告", "预增", "预亏", "净利润", "中报", "年报", "一季报", "三季报", "暴雷", "亏损", "扭亏", "披露"]),
    ("四级个股龙头", "并购重组", ["并购", "重组", "收购", "借壳", "资产注入"]),
    ("四级个股龙头", "月度经营数据", ["销量", "出货量", "交付量", "订单", "经营数据", "产销"]),
    ("四级个股龙头", "龙头产品发布", ["新品发布", "正式发布", "发布会", "首发", "上市首日", "申购", "询价", "招股", "新品", "公告"]),
    ("二级产业", "顶层产业扶持政策", ["扶持政策", "补贴", "行动方案", "指导意见", "产业政策", "支持.*产业", "人工智能\\+"]),
    ("二级产业", "行业监管调整", ["监管", "处罚", "新规", "立案", "国标", "强制性标准", "规范", "整治", "证监会"]),
    ("二级产业", "行业周期拐点", ["拐点", "景气", "涨价", "库存", "周期", "复苏", "回暖", "修复"]),
    ("二级产业", "产业重磅事件", ["峰会", "论坛", "博览会", "行业大会", "世界.*大会", "IPO", "量产", "突破", "签约", "开工", "投产", "首飞", "首航"]),
]
...
# v6.1 三层质量控制：
# 1) 财经门径：标题+摘要须命中下列财经词之一才进入分类（挡综合媒体的社会/文娱新闻）
FIN_GATE = ["股", "债", "基金", "央行", "证监会", "交易所", "银行", "保险", "券商", "信托",
            "经济", "GDP", "CPI", "PPI", "PMI", "利率", "汇率", "人民币", "美元", "金融",
            "资本", "投资", "融资", "贸易", "进出口", "出口", "进口", "产业", "行业", "企业",
            "上市", "IPO", "市值", "财报", "营收", "净利润", "价格", "油价", "金价", "楼市",
            "地产", "消费", "就业", "失业", "通胀", "财政", "货币", "关税", "制裁", "美联储",
            "国债", "ETF", "市场", "科技", "芯片", "半导体", "AI", "人工智能", "新能源",
            "医药", "汽车", "能源", "监管", "政策", "规划"]
# 2) 负面排除：标题命中即弃（仅当标题不含核心财经词时才启用，防"央行辟谣"类误杀）
NEG_TITLE = ["博物馆", "画展", "版画", "临展", "演出", "演唱会", "音乐节", "电影节",
             "电视剧", "综艺", "足球", "篮球", "世界杯", "奥运会", "世锦赛",
             "警方", "警情", "持枪", "抢劫", "盗窃", "谣言", "诈骗案"]
CORE_FIN = ["央行", "证监会", "股", "债", "基金", "美联储", "GDP", "CPI", "PPI", "PMI",
            "利率", "汇率", "交易所", "银保监", "金融监管总局"]
# 3) 权威国内信源排序加权（小时）：同等新旧程度下优先入选
FEED_BOOST_H = {"财联社·深度(RSSHub)": 6, "财联社·深度(镜像)": 6,
                "财联社·热门(RSSHub)": 6, "财联社·热门(镜像)": 6,
                "财新·最新(RSSHub)": 4, "财新·最新(镜像)": 4,
                "澎湃新闻(RSSHub)": 3, "澎湃新闻(镜像)": 3,
                "新浪财经·财经要闻": 2}
...
def main():
    ...
    seen, events = set(), []
    # 权威国内信源按小时加权：同等新旧下优先入选
    def sort_key(c):
        return c["when"] + dt.timedelta(hours=FEED_BOOST_H.get(c["feed"], 0))
    for c in sorted(cands, key=sort_key, reverse=True):
        ...
        text = c["title"] + " " + c["desc"]
        # v6.1 财经门径：综合媒体的社会/文娱新闻在此拦截
        if not any(k in text for k in FIN_GATE):
            continue
        # v6.1 负面排除：标题明显为社会/文娱新闻且不含核心财经词 → 弃
        if any(k in c["title"] for k in NEG_TITLE) and not any(k in c["title"] for k in CORE_FIN):
            continue
